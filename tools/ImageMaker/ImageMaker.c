...
int main(int argc, char* argv[]) {
	...
	if(argc < 3) {
		fprintf(stderr, "[ERROR] ImageMaker boot.bin kernel32.bin kernel64.bin\n");
		exit(-1);
	}
	...
	// Processing 64 Bit Protected Mode Kernel _ COPY TO DISK IMAGE FILE
	printf("[INFO] Copy IA-32e more kernel to image file\n");
...
void WriteKernelInformation(int iTargetFd, int iTotalKernelSectorCount ,int iKernel32SectorCount) {
	...
	printf("[INFO] Total sector count of exept boot loader [%d]\n", iTotalKernelSectorCount);
