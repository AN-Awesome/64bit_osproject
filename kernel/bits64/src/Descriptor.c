...
// SET THE VALUE IN THE 8-BYTE ENTRY
// FOR SET CODE/DATA DESCRIPTOR
...
// SET THE VALUE IN THE 16-BYTE ENTRY
// FOR SET TSS SEGMENT DESCRIPTOR
...
void kInitializeIDTTables(void) {
    ...
    for (int i = 21; i < 32; i++) kSetIDTEntry(&(pstEntry[i]), kISRETCException, 0x08, IDT_FLAGS_IST1, IDT_FLAGS_KERNEL, IDT_TYPE_INTERRUPT);
    ...
    for (int i = 48; i < IDT_MAXENTRYCOUNT; i++) kSetIDTEntry(&(pstEntry[i]), kISRETCInterrupt, 0x08, IDT_FLAGS_IST1, IDT_FLAGS_KERNEL, IDT_TYPE_INTERRUPT);
}

// SET A VALUE TO THE IDT GATE DESCRIPTOR
...
void kDummyHandler(void){
    kPrintString(0, 0, "===========================================", GREEN);
    kPrintString(0, 1, "    Dummy Interrupt Handler Execute~!!!    ", GREEN);
    kPrintString(0, 2, "     Interrupt or Exception Occur~!!!!     ", GREEN);
    kPrintString(0, 3, "===========================================", GREEN);
